...
def generate(spec_path):
    ...
    lines.append('void matmul_int8_rq(const int8_t *a, const int8_t *b, int8_t *out,')
    lines.append('                    int32_t M, int32_t K, int32_t N,')
    lines.append('                    int32_t scale, int32_t shift, int32_t zero_point);')
    lines.append('')

    layer_names = list(spec.keys())

    # emit frozen weight arrays
    ...
    for name, layer in spec.items():
        ...
        lines.append(f'// x: list of {cols} int8 values (one input vector)')
        lines.append(f'// returns: list of {rows} int8 values')
        ...
        lines.append(f'    matmul_int8_rq(weights_{name}, x_buf, out_buf, {rows}, {cols}, 1, scale, shift, zp);')
